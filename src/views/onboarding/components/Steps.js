    ...
    return (
      <div className="stepper">
        ...
        <div className="stepper-actions">
          <button className= 'genericbutton' onClick={() => this.previousStep()}>Previous</button>
          {activeStep === steps.length - 1 ?
            <button className= 'genericbutton' disabled={!!steps[activeStep].exitValidation} onClick={onFinish}>Submit</button> :
            <button className= 'genericbutton' disabled={!!steps[activeStep].exitValidation} onClick={() => this.nextStep()}>Next</button>}
        </div>
      </div>
    )
  }
