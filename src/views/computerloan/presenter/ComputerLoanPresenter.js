...
//POST
import AddLoanInteractor from '../../../domain/interactor/mpl/AddComputerLoanInteractor'
...
export default class ComputerLoanPresenter {
  ...
  getMplFormAttachments (formRequesting, nfisArray) {
    this.getFormAttachmentsInteractor.execute(MplGetFormParam(formRequesting, 5))
      .map(data => {
        let attachments = []
  ...
  getMplValidate () {
    ...
    this.view.showCircularLoader()
    this.getValidateInteractor.execute(MplValidateParam(5))
     .map(resp => {
        resp.modeOfLoan = []
